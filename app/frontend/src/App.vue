    ...
    <main>
        ...
        >
            <h1 class="text-lg font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-3">
                <span
                    >VersaDumps Visualizer <small v-if="currentVersion">v{{ currentVersion }}</small></span
                >
                <span
                    v-if="logs.length > 0"
                    class="inline-flex items-center justify-center bg-red-600 text-white text-xs font-semibold rounded-full w-6 h-6"
                    >{{ logs.length }}</span
                >
            </h1>
            <!--Buttons-->
            <div class="flex items-center gap-2">
                ...
                <div class="flex items-center gap-2 mr-2">
                    ...
                    <span class="text-xs text-slate-600 dark:text-slate-300" v-if="showServerText">{{
                        serverStatusText
                    }}</span>
                </div>
                <button class="icon-button" @click="toggleSortOrder" :title="sortButtonTitle">
                    <Icon name="sort" />
                </button>
                <button class="icon-button" @click="clearLogs" :title="t('clear_all_logs')">
                    <Icon name="trash" />
                </button>
                <button class="icon-button" @click="toggleTheme" :title="t('toggle_theme')">
                    <Icon :name="theme === 'dark' ? 'sun' : 'moon'" />
                </button>
                <button class="icon-button" @click="openConfigModal" :title="t('settings')">
                    <Icon name="gear" />
                </button>
                <!-- View toggle button -->
                <button
        ...
        <div class="split-container" :class="{ 'single-panel': !showLogFiles }">
            <!-- Top Panel: Dumps -->
            <div class="panel dumps-panel" :style="{ height: showLogFiles ? `${dumpsHeight}%` : '100%' }">
                    ...
                    <div v-if="logs.length === 0" class="text-center py-10 text-slate-500">
                        ...
                        <div class="mt-4">
                            <div class="mx-auto w-full max-w-2xl h-64 md:h-80 lg:h-96 relative">
                                <img
                                    src="./assets/images/versaDumpsVisualizer.webp"
                                    alt="versaDumpVisualizer"
                                    class="block w-full h-full object-contain"
                                />
                                <img
                                    src="./assets/images/texture.svg"
                                    alt=""
                                    class="pointer-events-none absolute inset-0 w-full h-full object-cover mix-blend-multiply opacity-80"
                                />
                            </div>
                        </div>
                    </div>
      ...
      */
            ...
            try {
                ...
                if (isMin && typeof Notification !== "undefined" && Notification.permission === "granted") {
                    const preview =
                        typeof parsedData.context === "string"
                            ? parsedData.context
                            : JSON.stringify(parsedData.context);
