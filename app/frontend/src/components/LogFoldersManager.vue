                ...
                <div class="flex items-start justify-between gap-2">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            />
                            <span
                                class="text-xs font-mono text-slate-700 dark:text-slate-300 truncate"
                                :title="folder.path || folder.Path"
                            >
                        ...
                        <div class="text-xs text-slate-500 dark:text-slate-400 space-y-0.5">
                            <div>
                                ...
                                {{
                                    ...
                                    (folder.extensions || folder.Extensions).length > 0
                                        ? (folder.extensions || folder.Extensions).join(", ")
                                        : t("all")
                                }}
                            </div>
                            <div>
                            ...
                            <div
                                v-if="
                                    (folder.filters || folder.Filters) && (folder.filters || folder.Filters).length > 0
                                "
                            >
        ...
        >
            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-6 max-w-md w-full mx-4" @click.stop>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            <label
                                v-for="level in ['error', 'warning', 'info', 'debug', 'success']"
                                :key="level"
                                class="flex items-center gap-1"
                            >
                                <input type="checkbox" :value="level" v-model="newFolder.filters" class="rounded" />
                ...
                <div class="mt-2 text-xs text-slate-500" v-if="false">
                    Path: "{{ newFolder.path }}" | Length: {{ newFolder.path?.length || 0 }} | Disabled:
                    {{ !newFolder.path || newFolder.path.trim() === "" }}
                </div>
            </div>
        </div>
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            <label
                                ...
                                class="flex items-center gap-1"
                            >
                                <input type="checkbox" :value="level" v-model="editingFolder.filters" class="rounded" />
                                <span class="text-sm text-slate-700 dark:text-slate-300">{{ t(level) }}</span>
                            </label>
                        </div>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
    ...
    try {
        ...
        if (e && e.message) {
            ...
            if (msg.includes("already exists")) {
                ...
                errorMessage = t.value("folder_not_exist");
            } else if (msg.includes("not a directory")) {
                errorMessage = t.value("folder_not_directory");
            } else if (msg.includes("permission") || msg.includes("access")) {
                errorMessage = t.value("folder_access_denied");
            } else {
