<template>
  <div class="log-file-viewer h-full flex flex-col bg-slate-50 dark:bg-slate-900">
    ...
    <div class="p-3 bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
      <div class="flex items-center justify-between mb-2">
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            :class="['text-xs px-2 py-1 rounded border', autoScroll ? 'bg-green-500 text-white border-green-600' : 'bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 text-slate-800 dark:text-slate-200']"
      ...
      <div class="flex flex-wrap gap-1" v-if="activeFiles.length > 0">
        <button
          ...
          :class="['text-xs px-2 py-1 rounded', fileFilters.includes(file) ? 'bg-blue-500 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300']"
        >
          {{ getFileName(file) }}
        </button>
      </div>
    ...
    <div
      ref="logContainer"
      class="flex-1 overflow-y-auto overflow-x-hidden font-mono text-xs p-2 space-y-0.5"
      @scroll="handleScroll"
    >
      <div
      ...
      <div
        ...
        :class="['p-1.5 rounded border-l-4', getLogLevelClass(log.level)]"
      >
        <div class="flex items-start gap-2">
          ...
          <span class="text-slate-500 dark:text-slate-400 text-[10px] shrink-0 truncate max-w-[150px]" :title="log.fileName">
            {{ log.fileName }}
          </span>
          <div class="text-slate-800 dark:text-slate-200 flex-1">
            <!-- Format JSON if applicable -->
            <pre v-if="log.isJson" class="json-content whitespace-pre-wrap text-[11px] leading-relaxed" v-html="log.coloredJson"></pre>
            <span v-else class="break-all">{{ log.line }}</span>
          </div>
        </div>
      </div>
    </div>
