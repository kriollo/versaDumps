...
- **Gestión de Perfiles**:
  - Implementación de un sistema robusto de perfiles múltiples (Crear, Activar, Eliminar).
  - Cada perfil mantiene su propia configuración de servidor, puerto, idioma y carpetas de log monitoreadas.
  - Sincronización en tiempo real mediante el evento `profileSwitched` para actualizar la UI sin recargar.
- **Sistema de Pruebas Automatizadas**:
  - Implementación de Vitest para el frontend con cobertura de componentes.
  - Implementación de pruebas unitarias en Go para el backend (`app_test.go`, `config_test.go`, `logwatcher_test.go`).
  - Scripts de ejecución de pruebas multiplataforma (`run-tests.ps1`, `run-tests.sh`).
- **Documentación Técnica**:
  - Nueva guía de inicio rápido para pruebas (`QUICK_START_TESTS.md`).
  - Documentación detallada del sistema de pruebas (`TESTING.md`).

### 🔧 Mejorado

- **Interfaz de Usuario (UI)**:
  - Rediseño del modal de configuración organizado por pestañas (General, Perfiles, Log Folders).
  - Notificaciones tipo "Toast" para feedback inmediato al cambiar perfiles o copiar logs.
  - Indicadores visuales de estado para el perfil activo.
  - Mayor reactividad en el toggle de "Mostrar tipos de variables".
- **Mantenimiento y Git**:
  - Actualización exhaustiva de `.gitignore` para excluir archivos de cobertura, logs temporales y archivos de suma de comprobación (`*.md5`).
  - Limpieza de archivos de configuración de Wails para distintas plataformas.
- **Traducciones**:
  - Actualización de archivos de idioma español (`es.js`) e inglés (`en.js`) con soporte para las nuevas funciones de perfiles.

### 📝 Técnico

- **Infraestructura de Pruebas**:
  - Configuración de Vitest (`vitest.config.js`) y entorno de setup (`test-setup.js`).
  - Generación de reportes de cobertura en Go (`coverage.out`) y Vue (`coverage/`).
...
- **Monitoreo de archivos de log**: Nueva funcionalidad completa para monitorear carpetas de archivos de log en tiempo real
  ...
  - Filtrado por patrones de nombres de archivo (errors*\*, access*\*, etc.)
